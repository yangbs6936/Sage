...
class FibreAgent(AgentBase):
    ...
    async def run_stream(
        ...
        if session_context and isinstance(
            getattr(session_context, "agent_config", None), dict
        ):
        ...
        if max_loop_count is None:
            raise ValueError(
                "FibreAgent requires session_context.agent_config.max_loop_count"
            )
        ...
        try:
            ...
            logger.error(
                f"FibreAgent: Error in run_stream: {e}\n{traceback.format_exc()}"
            )
            raise e
